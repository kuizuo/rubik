...
import { equalDirection, getAngleBetweenTwoVector2 } from "../util/math"
...
export class Cube extends Group {
  ...
  private createChildrenByData() {
    ...
    for (const element of this.data.elements) {
      const square = createSquare(new Color(element.color), element)
      this.add(square)
    }
  ...
  private getNeededRotateAngle() {
    ...
    let needRotateAnglePI =
      exceedAnglePI > rightAnglePI * 0.5 ? rightAnglePI - exceedAnglePI : -exceedAnglePI
